      ...
      <div ref={tooltipContentRef}>
        ...
        <div className="earthquake-tooltip__inline">
          <Field
            title="Date"
            hasMargin={false}
            value={formatDate(new Date(currentEarthquake.date))}
          />
          <Field
            title="Time"
            hasMargin={false}
            value={new Date(currentEarthquake.date).toISOString().substr(11, 8)}
          />
        </div>
        <div className="earthquake-tooltip__inline-flex">
          <Button kind="ghost" size="small" onClick={onHide}>
            Cancel
          </Button>
          <Button
            size="small"
