...
import {
  ...
  TableExpandHeader, TableExpandRow, TableHead, TableHeader,
...
const SensorsTable = () => {
  ...
  return <DataTable rows={formatRows(currentlyVisibleSensors)} headers={headers} className="sensors-table">
    {({
        rows,
        headers,
        getRowProps,
            ...
            <TableRow>
              ...
              {headers.map((header, headerIndex) => (
                <TableHeader
                  ...
                  className={headerIndex === 0 ? "sticky-column left" : undefined}
                >
                  {header.header}
                </TableHeader>
              ))}
                ...
                <Fragment key={row.id}>
                  <TableExpandRow {...getRowProps({row})}>
                    {row.cells.map((cell, indexCells) => (
                      <TableCell key={cell.id} className={indexCells === 0 ? "sticky-column left" : undefined}>
                        <span
                          tabIndex={0}
                        ...
                        >
                          {indexCells === 0 ?
                            ...
                            : Array.isArray(cell.value) ? formatCoordinates(cell.value) : cell.value
                          }
                        </span>
                        {indexCells === 0 && sensors[(page - 1) * pageSize + rowIndex].user_is_owner && (
                          <Tag
                            className="tag-owner"
                            tabIndex={0}
                    ...
                    <TableCell key={row.id + "-overflow"} className="sticky-column right">
                  ...
                  <TableExpandedRow colSpan={headers.length + 2} className="sensors-expandable-row">
...
const Sensors = () => {
  ...
  const zoomIn = () => {map.current.flyTo({zoom: map.current.getZoom() + 1})}
  const zoomOut = () => {map.current.flyTo({zoom: map.current.getZoom() - 1})}

  return (
   <div className="sensors-page">
     <p className="title" tabIndex={0}>Sensors</p>
     <Dropdown
     ...
     <div className="sensors-map__container">
       <div className="sensors-map__header">
         <h4 className="sensors-map__title" tabIndex={0}>Sensor locations</h4>
         <div className="sensors-map__controls">
           <span tabIndex={0} onClick={zoomIn} onKeyDown={(e) => keyboardOnlySubmit(e, zoomIn)}>
             <ZoomIn16/>
           </span>
           <span tabIndex={0} onClick={zoomOut} onKeyDown={(e) => keyboardOnlySubmit(e, zoomOut)}>
             <ZoomOut16/>
           </span>
         </div>
       </div>
